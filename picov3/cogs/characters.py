...
@client.tree.command(name="character-assign", description="Assign a character to your discord user")
@app_commands.describe(
    character_name="Name of the character",
    realm="Realm of the character"
)
async def assign(interaction: discord.Interaction, character_name: str, realm: str):
    try:
        await interaction.response.defer(thinking=True)
        character_name = character_name.capitalize()
        realm = normalizeRealmName(realm)
        ...
        if (db_entry := Character.get_or_none((Character.guild_id == interaction.guild.id) & (Character.name == character_name) & (Character.realm == realm))) is not None:
            db_entry: Character
            db_entry.user_id = interaction.user.id
            db_entry.save()            
            await interaction.edit_original_response(
                content=f"The character `{character_name}-{realm}` has been assigned to  <@{interaction.user.id}>"
            )
        else:            
            await interaction.edit_original_response(
                content=f"We couldn't find `{character_name}-{realm}` on our list.")
        ...
        logging.error(f"Assigning character failed for {character_name}-{realm}")
        traceback.print_exc()


@client.tree.command(name="character-unassign", description="Unassign a character that was linked to your discord user")
@app_commands.describe(
    character_name="Name of the character",
    realm="Realm of the character"
)
async def unassign(interaction: discord.Interaction, character_name: str, realm: str):
    try:
        await interaction.response.defer(thinking=True)
        character_name = character_name.capitalize()
        realm = normalizeRealmName(realm)
        ...
        if (db_entry := Character.get_or_none((Character.guild_id == interaction.guild.id) & (Character.name == character_name) & (Character.realm == realm))) is not None:
            ...
            db_entry.save()
            await interaction.edit_original_response(
                content=f"The character `{character_name}-{realm}` has been unassigned."
            )
        else:            
            await interaction.edit_original_response(
                content=f"We couldn't find `{character_name}-{realm}` on our list.")
        ...
        logging.error(f"Unassigning character failed for {character_name}-{realm}")
        traceback.print_exc()
